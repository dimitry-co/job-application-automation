...
Use `scripts/form-fill-cli.ts` for **all browser interactions**.

- Do **NOT** interact with the browser via raw Playwright calls or direct `page.evaluate` scripts.
- Use this workflow on each page:
  1. `snapshot` to inspect current fields and page text
  2. decide what to fill from profile + job content
  3. run `fill`, `dropdown`, `textarea`, `upload` as needed
  4. run `screenshot`
...
```bash
npx tsx scripts/form-fill-cli.ts snapshot
npx tsx scripts/form-fill-cli.ts fill --label "Name" --value "Jane Doe"
npx tsx scripts/form-fill-cli.ts dropdown --label "Degree" --search "Bachelor" --search "Computer Science"
npx tsx scripts/form-fill-cli.ts textarea --label "Why this company?" --answer "I am excited about your mission..."
npx tsx scripts/form-fill-cli.ts screenshot --name "after-personal-info"
npx tsx scripts/form-fill-cli.ts click --text "Continue"
```
